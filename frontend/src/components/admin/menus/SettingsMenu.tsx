...
function PermissionLevelsSection({ onSaved }: { onSaved: () => void }) {
  ...
  if (loading) return <div style={cardSt}><p style={titleSt}>권한 레벨 관리</p><p className="text-a13" style={{ color: UP.caption }}>로딩 중...</p></div>

  return (
    <div style={cardSt}>
      <p style={titleSt}>권한 레벨 관리 <span className="text-a11" style={{ color: UP.danger, fontWeight: 700, marginLeft: 8 }}>● 최고관리자 전용</span></p>
      <p className="text-a12" style={{ color: UP.sub, marginBottom: 16, lineHeight: 1.5 }}>
        ...
        <div key={key} style={{
          ...
          {editKey === key && editForm ? (
            <div>
              <div style={{ display: 'flex', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <label className="text-a11" style={{ color: UP.sub }}>색상</label>
                  <input
                    type="color"
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 }}>
                {FEATURE_KEYS.map(f => (
                  <label key={f.key} className="text-a11" style={{ display: 'flex', alignItems: 'center', gap: 4, color: UP.body, cursor: 'pointer' }}>
                    <input
                      type="checkbox"
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              <span className="text-a13" style={{ fontWeight: 700, color: lv.color, background: `${lv.color}18`, padding: '2px 10px', borderRadius: 999 }}>{lv.label}</span>
              <span className="text-a11" style={{ color: UP.caption }}>{key}</span>
      ...
      {msg && <p className="text-a12" style={{ color: msg.includes('실패') ? UP.danger : UP.green, marginTop: 8 }}>{msg}</p>}
    </div>
  )
}
...
function MaskingKeySection() {
  ...
  return (
    <div style={cardSt}>
      <p style={titleSt}>개인정보 마스킹 해제 보안키 <span className="text-a11" style={{ color: UP.danger, fontWeight: 700, marginLeft: 8 }}>● 최고관리자 전용</span></p>
      <p className="text-a12" style={{ color: UP.sub, marginBottom: 14, lineHeight: 1.5 }}>
      ...
      {!loading && (
        <div style={{ marginBottom: 14, padding: '10px 12px', borderRadius: 8, background: UP.sunken, border: `1px solid ${UP.hair}` }}>
          <div className="text-a11" style={{ color: UP.sub, marginBottom: 4 }}>현재 상태</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span className="text-a13" style={{ fontWeight: 700, color: configured ? UP.green : UP.caption }}>
              {configured ? '🔐 설정됨 (서버 해시 보관)' : '(미설정)'}
            </span>
            {configured && updatedAt && (
              <span className="text-a11" style={{ color: UP.caption }}>
      ...
      <label className="text-a12" style={{ color: UP.sub, display: 'block', marginBottom: 6 }}>
...
export default function SettingsMenu({ isSuperAdmin }: Props) {
  ...
  if (loading) return <div className="text-a14" style={{ padding: '40px', textAlign: 'center', color: UP.sub, }}>설정 로딩 중...</div>
    ...
    return (
      <div style={{ padding: '20px' }}>
        <div style={{ background: 'rgba(240,68,82,0.12)', border: '1px solid rgba(240,68,82,0.3)', borderRadius: 12, padding: '20px', color: UP.danger }}>
          ...
          <div className="text-a13" style={{ color: UP.body, marginBottom: 12 }}>{error || '데이터를 불러오지 못했습니다.'}</div>
          ...
          <>
            <div style={{ margin: '24px 0 14px', borderTop: `1px solid ${UP.hair}`, paddingTop: 20 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                ...
                <span className="text-a12" style={{ fontWeight: 700, color: UP.danger }}>최고관리자 전용 설정</span>
              </div>
              <p className="text-a11" style={{ color: UP.caption }}>catchmasterdmin@gmail.com 계정에서만 표시됩니다.</p>
            </div>
            <MaskingKeySection />
