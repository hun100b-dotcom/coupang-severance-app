  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <span style={{ fontSize: '0.78rem', color: '#0f172a', fontWeight: 600 }}>{d.name}</span>
      </div>
...
export default function ServiceBarChart({ severance, unemployment }: Props) {
  ...
  const data = [
    ...
    { name: '실업급여', value: unemployment, color: '#059669', pct: total ? Math.round(unemployment / total * 100) : 0 },
  ]
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: 12 }}>
        <p style={{ fontSize: '0.88rem', fontWeight: 700, color: '#0f172a', margin: 0 }}>서비스별 클릭</p>
        <p style={{ fontSize: '0.68rem', color: '#94a3b8', marginTop: 2 }}>총 {total.toLocaleString()}회</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 12 }}>
        {data.map(d => (
          <div key={d.name} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <div style={{ width: 8, height: 8, borderRadius: '50%', background: d.color }} />
            <span style={{ fontSize: '0.73rem', color: '#475569' }}>{d.name}</span>
