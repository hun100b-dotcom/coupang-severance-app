    ...
    <div className="w-full">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-[16px] font-extrabold text-up-navy">내 PDF 관리</h3>
        </div>
        <span className="text-[11px] text-up-sub font-semibold tabular-nums">
          {pdfs.length}/{MAX_PDFS}개
        </span>
      </div>

      {/* 내용 */}
      <div className="rounded-2xl bg-white border border-up-hair shadow-card overflow-hidden">
        {loading ? (
          <div className="flex items-center justify-center py-8 gap-2 text-up-sub">
            <Loader2 className="w-4 h-4 animate-spin" />
            <span className="text-[13px]">불러오는 중…</span>
          </div>
        ) : pdfs.length === 0 ? (
          <div className="text-center py-8 px-4">
            <FolderOpen className="w-10 h-10 text-up-caption mx-auto mb-2" />
            <p className="text-[14px] font-semibold text-up-sub">저장된 PDF가 없어요</p>
            <p className="text-[12px] text-up-caption mt-1">
              계산기에서 PDF를 업로드하면 저장할 수 있어요
            </p>
          </div>
        ) : (
          <div className="divide-y divide-up-hair">
            {pdfs.map(pdf => (
              <div key={pdf.id}
                className="flex items-center gap-3 px-4 py-3.5 hover:bg-up-sunken transition-colors">
                {/* 아이콘 */}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[14px] font-bold text-up-navy truncate">
                    {pdf.file_name}
                  </p>
                  <p className="text-[11px] text-up-sub tabular-nums">
