...
근거: 근로기준법 제55조, 제18조
주휴수당 = (주 소정근로시간 / 40) × 8 × 시급  (주 40시간 상한)
조건: 주 소정근로시간 15시간 이상 + 해당 주 소정근로일 개근

⚠️ 개근 조건: 근로내역서 PDF에는 '주별 결근(개근) 정보'가 없어, 여기서는 주 15시간
   이상인 주를 소정근로일 '개근'으로 **가정**해 산정한다(개근 여부는 코드로 검증하지 않음).
   → 프론트 정밀 결과에 '개근 가정' 안내를 노출해 무음 과대 산정을 방지한다.
"""
import traceback
