...
# 기본 계산 함수
# ──────────────────────────────────────────────────────────────────────────────
# 참고: 과거 MIN_ORDINARY_WAGE_DAILY(2021~2024 최저 통상임금 일급 하한) 상수가 있었으나,
#   평균임금 하한(통상임금 하한) 로직이 비활성 상태(is_ordinary_wage_applied 고정 False)라
#   어디서도 참조되지 않는 dead code였다. 통상임금 하한 활성화 계획이 없어 제거함.
#   (실업급여 구직급여일액 하한은 별도로 services/unemployment.py 에서 적용 중)
