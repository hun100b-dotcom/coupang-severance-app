...
## 제거 대상
개발자 개인 실명·영문명·닉네임 일체 (브랜드 'CATCH' 외 모든 개인 식별자)

## 수정 내역 (앱 가시 + 백엔드 응답 + 코드 주석)
| 파일 | 변경 |
|------|------|
| frontend/src/pages/Home.tsx:582 | 푸터 `© 2026 CATCH by [개인 닉네임]...` → `© 2026 CATCH. All rights reserved.` |
| frontend/src/pages/Intro.tsx:163 | 스플래시 하단 `by [개인 닉네임]` → `CATCH` |
| frontend/src/pages/PrivacyPolicy.tsx:326 | 개인정보 보호책임자 `성명: [개인 실명]` → `성명: CATCH 운영팀` |
| app.py:540 | Streamlit 푸터 `ⓒ 2026 [개인 닉네임]...` → `ⓒ 2026 CATCH...` |
| frontend/src/components/mypage/ProfileSection.tsx:38 | 주석의 개인 이름 예시 → `특정 사람 이름` |

## 총괄 리뷰어 A — PASS
- 정규식 전역 검색: frontend/src 0건, app.py 0건, index.html(meta/title/OG/JSON-LD) 0건.
- 영문 닉네임 단독도 frontend/src 0건(동일 철자 lucide 아이콘 import조차 없음 → 오탐 없음).
...
- 비가시 영역(당시 미션 범위 밖, 보고만): CLAUDE.md·memory·docs/marketing_kit·docs/launch·docs/privacy-policy-draft.md 등. **레포 공개 시 별도 검토 필요** — 종훈님 결정 대기 항목. (→ 후속 세션에서 일괄 스크럽 완료, `docs/dual_review/scrub_internal_docs.md` 참조)
