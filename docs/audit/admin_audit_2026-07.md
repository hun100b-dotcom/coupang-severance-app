...
### A-3. 지표 부풀림의 진짜 정체 = 봇(프리렌더) (2026-07-06, 커밋 `9c6d5ef`)
종훈님 "수치가 너무 높다·가짜 같다" 재신고 → 실 DB 전수 분석으로 확정:
- 최근 30일 `visitor_logs` **4,131건 중 2,482건(60%)이 봇/자동화**. 카테고리 실측:
  **HeadlessChrome(프리렌더) 2,440건(1,484세션)** + 검색봇 42 + 사람 1,649건(270세션).
- **주범 = Vercel 배포 시 `scripts/prerender.mjs`의 헤드리스 크롬**이 SEO 17라우트를 렌더할 때
  `useVisitorTracking` 훅이 실행돼 visitor_logs 에 매 배포마다 방문을 남김. 즉 순방문자 "1,796"의
  실체는 **실유저 270**, 나머지는 봇.
- **실유저 재정의**: 봇/헤드리스/크롤러/모니터 제외 + 순방문자=distinct 세션. 수집 차단
  (navigator.webdriver·봇 UA면 insert 스킵) + 백엔드 집계 제외(_is_bot_ua) + bots_excluded/raw 투명 표기.
  과거 봇행은 삭제 안 함(로우데이터 보존), 집계에서만 제외.
- **before→after(라이브)**: 총페이지뷰 4,046→**1,649**, 순방문자 1,796→**270**, 로그인 실인원 15, 봇 2,482 제외.
