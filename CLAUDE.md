...
## 🟥 상시규칙 (종훈님 명시 — 모든 세션 영구 적용, 절대 축약·삭제 금지)

> 2026-06-30 어드민 재개발 지시와 함께 등록. 아래 4개 규칙은 모든 작업에 우선 적용된다.

### [규칙1] "방법없음/불가능" 보고 금지

"불가능 / 방법없음 / 도구없음" 이라는 결론을 내기 전, **아래 4가지를 반드시 먼저 시도**하고 각 시도의 명령·코드·에러 원문을 로그로 남긴다:
- (a) 헤드리스(Playwright/Puppeteer)로 `getComputedStyle` 실측
- (b) 크롬 도구로 DOM computed 값 추출
- (c) 빌드된 CSS를 정적 파싱해 폰트 규칙 계산·대조
- (d) 웹 검색으로 방법 탐색

네 가지를 다 시도하기 전에는 "불가능" 이라고 말하지 않는다. 막히면 반드시 **[시도한 것 + 에러 원문 / 막힌 이유 / 다음 시도(최소 1개)]** 형식으로만 보고. **설치 권한·의존성 부재는 "불가능" 사유가 아니다**(헤드리스·정적파싱 등 우회).

### [규칙2] 엄격 더블리뷰 (빌더 + 리뷰어, 5축)

모든 구현은 [빌더] 후 [리뷰어]가 아래 5축으로 검수하고, 걸리면 **통과할 때까지 재수정**한다. 보고 끝에 **[리뷰어 판정]** 을 명시한다.
1. **디자인**: 토큰 완전 일치 · 무지개색 금지(블루+그린+회색만)
2. **UI**: 정렬·간격·radius·shadow · 공통 컴포넌트(`ui/*`) 재사용
3. **UX**: 진입→작업→결과→예외 + 로딩/성공/실패/빈상태 4상태
4. **코드**: 중복·죽은코드·인라인 style 잔존·타입·한국어 주석
5. **회귀**: 측정값(규칙3)

### [규칙3] 회귀는 "측정값"으로만 판정

전역 영향(특히 폰트) 변경 시: 변경 **전** 대표 요소들의 `computed font-size(px)`를 baseline으로 기록 → 변경 **후** 재측정 → **1px이라도 다르면 자동 FAIL → 즉시 롤백**. 전 항목 일치할 때만 PASS. 보고에 **전/후 수치표 + 측정 명령 + 원본 출력**을 첨부한다.
- ⚠️ 최우선 함정: `index.css`의 폰트 전역 `+1.5px !important` override (회귀 1순위 감시 대상).

### [규칙4] 절대 / 조건부

- **(절대)** 계산 로직(28일 블록) 변경 금지. 배포는 `git push origin main` → Vercel/Render 자동.
- **(조건부)** `toss.*` 토큰: **무조건 삭제가 아니라**, 업비트 전환이 끝났으니 **Toss 잔재 청산이 목표**. 단 **참조 관계를 먼저 조사** → 안전 제거 가능 여부 판단 후 **순차 정리**. 조사 없는 일괄 삭제만 금지.
...
### 워크플로우
- 모든 응답: 한글 필수
- 승인 요청 없이 즉시 실행 (파일 삭제·구조 변경 포함 전부 자율 진행. 종훈님이 이미 승인한 큰 방향은 스텝마다 재확인하지 말 것)
...
### 작업 워크플로우 (자율 진행 — 중간에 멈추지 말 것)
- **큰 작업은 먼저 플랜을 제시**하되, 종훈님이 승인하면 **모든 스텝을 연속 자율 진행**한다. 스텝마다 멈춰서 승인을 다시 묻지 않는다.
- **절대 "승인 대기"로 멈추지 않는다** — 승인은 작업 시작 시 한 번. 이후 각 스텝은 [빌더 → 더블리뷰 5축 → 회귀 측정 → 커밋·배포] 사이클을 스스로 돌리고, 끝날 때마다 짧게 보고만 하고 바로 다음 스텝으로 넘어간다.
- **품질 게이트(더블리뷰 5축·회귀 측정)는 유지**하되, 이것 때문에 사용자에게 되묻거나 정지하지 않는다. FAIL이면 스스로 수정 후 재검증.
- **치명 이슈만 에스컬레이션**: 운영 DB 데이터 변경 필요, 계산 로직 충돌, 폰트 회귀 발생, 롤백 필요 — 이 4가지만 종훈님께 보고하고 판단 요청. 그 외는 전부 자율.
- **Cowork/Dispatch 환경에서도 동일** — 서브태스크는 "끝까지 연속 진행" 지시로 넘긴다.
...
- (참고) 파일 수 제한·plan 게이트로 인한 "중간 정지"는 폐지됨 — 큰 작업도 승인 후 끝까지 자율 진행
