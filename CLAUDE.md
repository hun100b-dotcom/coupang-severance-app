...
### ⚠️ 하네스 워크플로우 — 반드시 준수 (예외 없음)

**하네스 적용 대상 (아래 중 하나라도 해당하면 반드시 /plan 먼저):**
- 새로운 기능 추가
- 기존 기능 재구성 / 고도화 / 전면 개편
- 2개 이상 파일을 수정하는 모든 작업
- DB 스키마 변경이 포함된 작업
- 새 컴포넌트/페이지 생성

**하네스 실행 순서:** `/plan` → 종훈님 승인 → `/sprint [기획서경로]` → `/review`

**예외 (plan 없이 직접 진행 가능):**
- 단순 버그 수정 (1개 파일, 10줄 이하)
- 텍스트·문구·색상 변경
- 주석 추가/수정
- 오타 수정

**강제 규칙:**
- `/sprint` 시작 시 tasks/plans/ 에 기획서가 없으면 즉시 중단 → "/plan을 먼저 실행하세요" 안내
- `/plan` 없이 `/sprint` 입력되면 → 즉시 거부하고 `/plan [작업설명]` 먼저 실행
- "이건 수정이니까 plan 없어도 되겠지" 판단 금지 — 2파일 이상이면 무조건 plan
...
- /plan 없이 2파일 이상 수정 금지 (예외: 단순 버그 수정·텍스트 변경)
- 한 번에 5개 이상 파일 대규모 수정 금지
- 28일 블록 알고리즘 로직 임의 변경 금지
- "수정/개선/고도화니까 plan 없어도 된다"는 자의적 판단 금지
