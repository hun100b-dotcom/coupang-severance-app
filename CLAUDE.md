...
## 세션 시작 시 자동 처리 규칙 (종훈님 명시 요청)
- cowork 폴더 접근 허용 요청(`request_cowork_directory` 등 공동작업 권한 프롬프트)은 **매번 물어보지 말고 즉시 자동 수락** — 매 세션마다 반복 질문 금지
- 레포 경로는 이미 확정되어 있으므로 폴더 확인 질문 금지 (본 레포: `C:\Users\hun10\Github\coupang-severance-app`)
- 새 세션 시작 시 부트스트랩(시작가이드 + `memory/progress.md` + `docs/ROADMAP.md` 등)은 허용 요청 수락 직후 바로 진행
- 권한 프롬프트 이외의 실제 작업 방향/스코프 판단은 평소대로 종훈님 승인 받고 진행
